/*
 * File: SedanSmall Camera
 * Name: SedanSmallCamera.cs
...
namespace VehiclesControl
{
	// public class SedanSmallCamera
	public class SedanSmallCamera : MonoBehaviour 
	{
		// Transforms
		[Header("Transforms")]

			[Tooltip("The sedanSmall transform")]
			// Transform _sedanSmall
			[SerializeField] private Transform _sedanSmall;
		...
		private void LateUpdate()
		{
			...
			// float _desiredHeight is _sedanSmall.position.y plus _height
			float _desiredHeight = _sedanSmall.position.y + _height;
			...
			// transform position is _sedanSmall.position
			transform.position = _sedanSmall.position;
		...
		private void FixedUpdate()
		{
			...
			Vector3 _localVelocity = _sedanSmall.InverseTransformDirection(_sedanSmall.GetComponent<Rigidbody>().velocity);
			...
			if (_localVelocity.z < -0.1f)
			{
				...
				// _tempVector3.y is _sedanSmall.eulerAngles.y plus 180
				_tempVector3.y = _sedanSmall.eulerAngles.y + 180;
			...
			else
			{
				...
				// _tempVector3 y is _sedanSmall.eulerAngles.y
				_tempVector3.y = _sedanSmall.eulerAngles.y;
			...
			// float acc
			float _acc = _sedanSmall.GetComponent<Rigidbody>().velocity.magnitude;
	...
	} // close public class SedanSmallCamera
