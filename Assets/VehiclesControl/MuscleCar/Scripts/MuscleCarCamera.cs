/*
 * File: MuscleCar Camera
 * Name: MuscleCarCamera.cs
...
namespace VehiclesControl
{
	// public class MuscleCarCamera
	public class MuscleCarCamera : MonoBehaviour 
	{
		// Transforms
		[Header("Transforms")]

			[Tooltip("The muscleCar transform")]
			// Transform _muscleCar
			[SerializeField] private Transform _muscleCar;
		...
		private void LateUpdate()
		{
			...
			// float _desiredHeight is _muscleCar.position.y plus _height
			float _desiredHeight = _muscleCar.position.y + _height;
			...
			// transform position is _muscleCar.position
			transform.position = _muscleCar.position;
		...
		private void FixedUpdate()
		{
			...
			Vector3 _localVelocity = _muscleCar.InverseTransformDirection(_muscleCar.GetComponent<Rigidbody>().velocity);
			...
			if (_localVelocity.z < -0.1f)
			{
				...
				// _tempVector3.y is _muscleCar.eulerAngles.y plus 180
				_tempVector3.y = _muscleCar.eulerAngles.y + 180;
			...
			else
			{
				...
				// _tempVector3 y is _muscleCar.eulerAngles.y
				_tempVector3.y = _muscleCar.eulerAngles.y;
			...
			// float acc
			float _acc = _muscleCar.GetComponent<Rigidbody>().velocity.magnitude;
	...
	} // close public class MuscleCarCamera
