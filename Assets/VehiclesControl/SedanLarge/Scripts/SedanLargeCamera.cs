/*
 * File: SedanLarge Camera
 * Name: SedanLargeCamera.cs
...
namespace VehiclesControl
{
	// public class SedanLargeCamera
	public class SedanLargeCamera : MonoBehaviour 
	{
		// Transforms
		[Header("Transforms")]

			[Tooltip("The sedanLarge transform")]
			// Transform _sedanLarge
			[SerializeField] private Transform _sedanLarge;
		...
		private void LateUpdate()
		{
			...
			// float _desiredHeight is _sedanLarge.position.y plus _height
			float _desiredHeight = _sedanLarge.position.y + _height;
			...
			// transform position is _sedanLarge.position
			transform.position = _sedanLarge.position;
		...
		private void FixedUpdate()
		{
			...
			Vector3 _localVelocity = _sedanLarge.InverseTransformDirection(_sedanLarge.GetComponent<Rigidbody>().velocity);
			...
			if (_localVelocity.z < -0.1f)
			{
				...
				// _tempVector3.y is _sedanLarge.eulerAngles.y plus 180
				_tempVector3.y = _sedanLarge.eulerAngles.y + 180;
			...
			else
			{
				...
				// _tempVector3 y is _sedanLarge.eulerAngles.y
				_tempVector3.y = _sedanLarge.eulerAngles.y;
			...
			// float acc
			float _acc = _sedanLarge.GetComponent<Rigidbody>().velocity.magnitude;
	...
	} // close public class SedanLargeCamera
