/*
 * File: SedanMedium Camera
 * Name: SedanMediumCamera.cs
...
namespace VehiclesControl
{
	// public class SedanMediumCamera
	public class SedanMediumCamera : MonoBehaviour 
	{
		// Transforms
		[Header("Transforms")]

			[Tooltip("The sedanMedium transform")]
			// Transform _sedanMedium
			[SerializeField] private Transform _sedanMedium;
		...
		private void LateUpdate()
		{
			...
			// float _desiredHeight is _sedanMedium.position.y plus _height
			float _desiredHeight = _sedanMedium.position.y + _height;
			...
			// transform position is _sedanMedium.position
			transform.position = _sedanMedium.position;
		...
		private void FixedUpdate()
		{
			...
			Vector3 _localVelocity = _sedanMedium.InverseTransformDirection(_sedanMedium.GetComponent<Rigidbody>().velocity);
			...
			if (_localVelocity.z < -0.1f)
			{
				...
				// _tempVector3.y is _sedanMedium.eulerAngles.y plus 180
				_tempVector3.y = _sedanMedium.eulerAngles.y + 180;
			...
			else
			{
				...
				// _tempVector3 y is _sedanMedium.eulerAngles.y
				_tempVector3.y = _sedanMedium.eulerAngles.y;
			...
			// float acc
			float _acc = _sedanMedium.GetComponent<Rigidbody>().velocity.magnitude;
	...
	} // close public class SedanMediumCamera
