/*
 * File: Ambulance Camera
 * Name: AmbulanceCamera.cs
...
namespace VehiclesControl
{
	// public class AmbulanceCamera
	public class AmbulanceCamera : MonoBehaviour 
	{
		// Transforms
		[Header("Transforms")]

			[Tooltip("The ambulance transform")]
			// Transform _ambulance
			[SerializeField] private Transform _ambulance;
		...
		private void LateUpdate()
		{
			...
			// float _desiredHeight is _ambulance.position.y plus _height
			float _desiredHeight = _ambulance.position.y + _height;
			...
			// transform position is _ambulance.position
			transform.position = _ambulance.position;
		...
		private void FixedUpdate()
		{
			...
			Vector3 _localVelocity = _ambulance.InverseTransformDirection(_ambulance.GetComponent<Rigidbody>().velocity);
			...
			if (_localVelocity.z < -0.1f)
			{
				...
				// _tempVector3.y is _ambulance.eulerAngles.y plus 180
				_tempVector3.y = _ambulance.eulerAngles.y + 180;
			...
			else
			{
				...
				// _tempVector3 y is _ambulance.eulerAngles.y
				_tempVector3.y = _ambulance.eulerAngles.y;
			...
			// float acc
			float _acc = _ambulance.GetComponent<Rigidbody>().velocity.magnitude;
	...
	} // close public class AmbulanceCamera
