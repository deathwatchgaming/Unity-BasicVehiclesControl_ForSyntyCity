...
namespace VehiclesControl
{
	// public class TaxiCamera
	public class TaxiCamera : MonoBehaviour 
	{
		// Transforms
		[Header("Transforms")]

			[Tooltip("The taxi transform")]
			// Transform _taxi
			[SerializeField] private Transform _taxi;
		...
		private void LateUpdate()
		{
			...
			// float _desiredHeight is _taxi.position.y plus _height
			float _desiredHeight = _taxi.position.y + _height;
			...
			// transform position is _taxi.position
			transform.position = _taxi.position;
		...
		private void FixedUpdate()
		{
			...
			Vector3 _localVelocity = _taxi.InverseTransformDirection(_taxi.GetComponent<Rigidbody>().velocity);
			...
			if (_localVelocity.z < -0.1f)
			{
				...
				// _tempVector3.y is _taxi.eulerAngles.y plus 180
				_tempVector3.y = _taxi.eulerAngles.y + 180;
			...
			else
			{
				...
				// _tempVector3 y is _taxi.eulerAngles.y
				_tempVector3.y = _taxi.eulerAngles.y;
			...
			// float acc
			float _acc = _taxi.GetComponent<Rigidbody>().velocity.magnitude;
