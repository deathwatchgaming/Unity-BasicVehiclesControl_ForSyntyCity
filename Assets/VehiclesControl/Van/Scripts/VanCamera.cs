...
namespace VehiclesControl
{
	// public class VanCamera
	public class VanCamera : MonoBehaviour 
	{
		// Transforms
		[Header("Transforms")]
		...
		private void LateUpdate()
		{
			...
			// float _desiredHeight is _van.position.y plus _height
			float _desiredHeight = _van.position.y + _height;
			...
			// transform position is _van.position
			transform.position = _van.position;
		...
		private void FixedUpdate()
		{
			...
			Vector3 _localVelocity = _van.InverseTransformDirection(_van.GetComponent<Rigidbody>().velocity);
			...
			if (_localVelocity.z < -0.1f)
			{
				...
				// _tempVector3.y is _van.eulerAngles.y plus 180
				_tempVector3.y = _van.eulerAngles.y + 180;
			...
			else
			{
				...
				// _tempVector3 y is _van.eulerAngles.y
				_tempVector3.y = _van.eulerAngles.y;
			...
			// float acc
			float _acc = _van.GetComponent<Rigidbody>().velocity.magnitude;
