/*
 * File: Police Cruiser Camera
 * Name: PoliceCruiserCamera.cs
...
namespace VehiclesControl
{
	// public class PoliceCruiserCamera
	public class PoliceCruiserCamera : MonoBehaviour 
	{
		// Transforms
		[Header("Transforms")]
		...
		private void LateUpdate()
		{
			...
			// float _desiredHeight is _policeCruiser.position.y plus _height
			float _desiredHeight = _policeCruiser.position.y + _height;
			...
			// transform position is _policeCruiser.position
			transform.position = _policeCruiser.position;
			...
			// transform LookAt _policeCruiser
			transform.LookAt(_policeCruiser);
		...
		private void FixedUpdate()
		{
			...
			Vector3 _localVelocity = _policeCruiser.InverseTransformDirection(_policeCruiser.GetComponent<Rigidbody>().velocity);
			...
			if (_localVelocity.z < -0.1f)
			{
				...
				// _tempVector3.y is _policeCruiser.eulerAngles.y plus 180
				_tempVector3.y = _policeCruiser.eulerAngles.y + 180;
			...
			else
			{
				...
				// _tempVector3 y is _policeCruiser.eulerAngles.y
				_tempVector3.y = _policeCruiser.eulerAngles.y;
			...
			// float acc
			float _acc = _policeCruiser.GetComponent<Rigidbody>().velocity.magnitude;
	...
	} // close public class PoliceCruiserCamera
