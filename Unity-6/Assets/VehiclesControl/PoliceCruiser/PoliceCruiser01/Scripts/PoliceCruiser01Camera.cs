/*
 * File: Police Cruiser 01 Camera
 * Name: PoliceCruiser01Camera.cs
...
namespace VehiclesControl
{
	// public class PoliceCruiser01Camera
	public class PoliceCruiser01Camera : MonoBehaviour 
	{
		// Transforms
		[Header("Transforms")]

			[Tooltip("The police cruiser 01 transform")]
		...
		private void LateUpdate()
		{
			...
			// float _desiredHeight is _policeCruiser01.position.y plus _height
			float _desiredHeight = _policeCruiser01.position.y + _height;
			...
			// transform position is _policeCruiser01.position
			transform.position = _policeCruiser01.position;
			...
			// transform LookAt _policeCruiser01
			transform.LookAt(_policeCruiser01);
		...
		private void FixedUpdate()
		{
			...
			Vector3 _localVelocity = _policeCruiser01.InverseTransformDirection(_policeCruiser01.GetComponent<Rigidbody>().linearVelocity);
			...
			if (_localVelocity.z < -0.1f)
			{
				...
				// _tempVector3.y is _policeCruiser01.eulerAngles.y plus 180
				_tempVector3.y = _policeCruiser01.eulerAngles.y + 180;
			...
			else
			{
				...
				// _tempVector3 y is _policeCruiser01.eulerAngles.y
				_tempVector3.y = _policeCruiser01.eulerAngles.y;
			...
			float _autoCamControl = _policeCruiser01.GetComponent<Rigidbody>().linearVelocity.magnitude;
	...
	} // close public class PoliceCruiser01Camera
