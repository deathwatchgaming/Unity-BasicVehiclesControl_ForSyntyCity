/*
 * File: Police Cruiser 04 Camera
 * Name: PoliceCruiser04Camera.cs
...
namespace VehiclesControl
{
	// public class PoliceCruiser04Camera
	public class PoliceCruiser04Camera : MonoBehaviour 
	{
		// Transforms
		[Header("Transforms")]

			[Tooltip("The police cruiser 04 transform")]
		...
		private void LateUpdate()
		{
			...
			// float _desiredHeight is _policeCruiser04.position.y plus _height
			float _desiredHeight = _policeCruiser04.position.y + _height;
			...
			// transform position is _policeCruiser04.position
			transform.position = _policeCruiser04.position;
			...
			// transform LookAt _policeCruiser04
			transform.LookAt(_policeCruiser04);
		...
		private void FixedUpdate()
		{
			...
			Vector3 _localVelocity = _policeCruiser04.InverseTransformDirection(_policeCruiser04.GetComponent<Rigidbody>().linearVelocity);
			...
			if (_localVelocity.z < -0.1f)
			{
				...
				// _tempVector3.y is _policeCruiser04.eulerAngles.y plus 180
				_tempVector3.y = _policeCruiser04.eulerAngles.y + 180;
			...
			else
			{
				...
				// _tempVector3 y is _policeCruiser04.eulerAngles.y
				_tempVector3.y = _policeCruiser04.eulerAngles.y;
			...
			float _autoCamControl = _policeCruiser04.GetComponent<Rigidbody>().linearVelocity.magnitude;
	...
	} // close public class PoliceCruiser04Camera
