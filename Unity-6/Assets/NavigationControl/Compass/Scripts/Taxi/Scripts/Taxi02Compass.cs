/*
*  Name: Taxi 02 Compass
*  File: Taxi02Compass.cs
...
namespace NavigationControl
{
	// public class Taxi02Compass
	public class Taxi02Compass : MonoBehaviour
	{
		// Header
		[Header("Raw Images")]
		...
		[Header("Transform")]
			...
			// Tooltip
			[Tooltip("The Taxi02 transform")]
			// Transform _taxi02
			[SerializeField] private Transform _taxi02;
		...
		public static Taxi02Compass _taxi02Compass;
		...
		private void Update()
		{
			...
			if (compassEnabled == true)
			{
				...
				// Get Component Taxi02Compass is enabled
				GetComponent<Taxi02Compass>().enabled = true;

				// Debug Log Taxi02 compass is enabled
			...
			{
				// Debug Log Taxi02 compass is disabled
				...
				// Get Component Taxi02Compass is not enabled
				GetComponent<Taxi02Compass>().enabled = false;

			} // close else if compassEnabled is false

			// Get a handle on the image's uvRect

			// _compassForeground uvRect is new Rect _taxi02 localEulerAngles.y 360, 0, 1, 1
			_compassForeground.uvRect = new Rect(_taxi02.localEulerAngles.y / 360, 0, 1, 1);

			// Get a copy of the _taxi02 forward vector

			// Vector3 forward is _taxi02 transform forward
			Vector3 forward = _taxi02.transform.forward;

			// Zero out the Y of _taxi02 forward vector to only get the direction in the X,Z 
	...
	} // close public class Taxi02Compass
