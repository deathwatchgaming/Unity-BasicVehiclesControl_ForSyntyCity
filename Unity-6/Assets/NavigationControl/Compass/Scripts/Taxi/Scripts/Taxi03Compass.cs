/*
*  Name: Taxi 03 Compass
*  File: Taxi03Compass.cs
...
namespace NavigationControl
{
	// public class Taxi03Compass
	public class Taxi03Compass : MonoBehaviour
	{
		// Header
		[Header("Raw Images")]
		...
		[Header("Transform")]
			...
			// Tooltip
			[Tooltip("The Taxi03 transform")]
			// Transform _taxi03
			[SerializeField] private Transform _taxi03;
		...
		public static Taxi03Compass _taxi03Compass;
		...
		private void Update()
		{
			...
			if (compassEnabled == true)
			{
				...
				// Get Component Taxi03Compass is enabled
				GetComponent<Taxi03Compass>().enabled = true;

				// Debug Log Taxi03 compass is enabled
			...
			{
				// Debug Log Taxi03 compass is disabled
				...
				// Get Component Taxi03Compass is not enabled
				GetComponent<Taxi03Compass>().enabled = false;

			} // close else if compassEnabled is false

			// Get a handle on the image's uvRect

			// _compassForeground uvRect is new Rect _taxi03 localEulerAngles.y 360, 0, 1, 1
			_compassForeground.uvRect = new Rect(_taxi03.localEulerAngles.y / 360, 0, 1, 1);

			// Get a copy of the _taxi03 forward vector

			// Vector3 forward is _taxi03 transform forward
			Vector3 forward = _taxi03.transform.forward;

			// Zero out the Y of _taxi03 forward vector to only get the direction in the X,Z 
	...
	} // close public class Taxi03Compass
