/*
*  Name: Muscle Car 04 Compass
*  File: MuscleCar04Compass.cs
...
namespace NavigationControl
{
	// public class MuscleCar04Compass
	public class MuscleCar04Compass : MonoBehaviour
	{
		// Header
		[Header("Raw Images")]
		...
		[Header("Transform")]
			...
			// Tooltip
			[Tooltip("The MuscleCar04 transform")]
			// Transform _muscleCar04
			[SerializeField] private Transform _muscleCar04;
		...
		public static MuscleCar04Compass _muscleCar04Compass;

		// private void Start
		private void Start()
		{
			// _muscleCar04Compass
			_muscleCar04Compass = this;
		...
		private void Update()
		{
			...
			if (compassEnabled == true)
			{
				...
				// Get Component MuscleCar04Compass is enabled
				GetComponent<MuscleCar04Compass>().enabled = true;

				// Debug Log MuscleCar04 compass is enabled
			...
			{
				// Debug Log MuscleCar04 compass is disabled
				...
				// Get Component MuscleCar04Compass is not enabled
				GetComponent<MuscleCar04Compass>().enabled = false;

			} // close else if compassEnabled is false

			// Get a handle on the image's uvRect

			// _compassForeground uvRect is new Rect _muscleCar04 localEulerAngles.y 360, 0, 1, 1
			_compassForeground.uvRect = new Rect(_muscleCar04.localEulerAngles.y / 360, 0, 1, 1);

			// Get a copy of the _muscleCar04 forward vector

			// Vector3 forward is _muscleCar04 transform forward
			Vector3 forward = _muscleCar04.transform.forward;

			// Zero out the Y of _muscleCar04 forward vector to only get the direction in the X,Z 
	...
	} // close public class MuscleCar04Compass
