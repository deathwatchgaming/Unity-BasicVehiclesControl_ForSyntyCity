/*
 * File: Ambulance 04 Camera
 * Name: Ambulance04Camera.cs
...
namespace VehiclesControl
{
	// public class Ambulance04Camera
	public class Ambulance04Camera : MonoBehaviour 
	{
		// Transforms
		[Header("Transforms")]
		...
		private void LateUpdate()
		{
			...
			// float _desiredHeight is _ambulance04.position.y plus _height
			float _desiredHeight = _ambulance04.position.y + _height;
			...
			// transform position is _ambulance04.position
			transform.position = _ambulance04.position;
		...
		private void FixedUpdate()
		{
			...
			Vector3 _localVelocity = _ambulance04.InverseTransformDirection(_ambulance04.GetComponent<Rigidbody>().linearVelocity);
			...
			if (_localVelocity.z < -0.1f)
			{
				...
				// _tempVector3.y is _ambulance04.eulerAngles.y plus 180
				_tempVector3.y = _ambulance04.eulerAngles.y + 180;
			...
			else
			{
				...
				// _tempVector3 y is _ambulance04.eulerAngles.y
				_tempVector3.y = _ambulance04.eulerAngles.y;
			...
			float _autoCamControl = _ambulance04.GetComponent<Rigidbody>().linearVelocity.magnitude;
	...
	} // close public class Ambulance04Camera
