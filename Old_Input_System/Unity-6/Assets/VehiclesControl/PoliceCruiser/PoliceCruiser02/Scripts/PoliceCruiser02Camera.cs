/*
 * File: Police Cruiser 02 Camera
 * Name: PoliceCruiser02Camera.cs
...
namespace VehiclesControl
{
	// public class PoliceCruiser02Camera
	public class PoliceCruiser02Camera : MonoBehaviour 
	{
		// Transforms
		[Header("Transforms")]

			[Tooltip("The police cruiser 02 transform")]
		...
		private void LateUpdate()
		{
			...
			// float _desiredHeight is _policeCruiser02.position.y plus _height
			float _desiredHeight = _policeCruiser02.position.y + _height;
			...
			// transform position is _policeCruiser02.position
			transform.position = _policeCruiser02.position;
			...
			// transform LookAt _policeCruiser02
			transform.LookAt(_policeCruiser02);
		...
		private void FixedUpdate()
		{
			...
			Vector3 _localVelocity = _policeCruiser02.InverseTransformDirection(_policeCruiser02.GetComponent<Rigidbody>().linearVelocity);
			...
			if (_localVelocity.z < -0.1f)
			{
				...
				// _tempVector3.y is _policeCruiser02.eulerAngles.y plus 180
				_tempVector3.y = _policeCruiser02.eulerAngles.y + 180;
			...
			else
			{
				...
				// _tempVector3 y is _policeCruiser02.eulerAngles.y
				_tempVector3.y = _policeCruiser02.eulerAngles.y;
			...
			float _autoCamControl = _policeCruiser02.GetComponent<Rigidbody>().linearVelocity.magnitude;
	...
	} // close public class PoliceCruiser02Camera
