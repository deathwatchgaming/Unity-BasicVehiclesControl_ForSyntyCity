...
namespace VehiclesControl
{
	// public class Ambulance02Camera
	public class Ambulance02Camera : MonoBehaviour 
	{
		// Transforms
		[Header("Transforms")]
		...
		private void LateUpdate()
		{
			...
			// float _desiredHeight is _ambulance02.position.y plus _height
			float _desiredHeight = _ambulance02.position.y + _height;
			...
			// transform position is _ambulance02.position
			transform.position = _ambulance02.position;
		...
		private void FixedUpdate()
		{
			...
			Vector3 _localVelocity = _ambulance02.InverseTransformDirection(_ambulance02.GetComponent<Rigidbody>().velocity);
			...
			if (_localVelocity.z < -0.1f)
			{
				...
				// _tempVector3.y is _ambulance02.eulerAngles.y plus 180
				_tempVector3.y = _ambulance02.eulerAngles.y + 180;
			...
			else
			{
				...
				// _tempVector3 y is _ambulance02.eulerAngles.y
				_tempVector3.y = _ambulance02.eulerAngles.y;
			...
			float _autoCamControl = _ambulance02.GetComponent<Rigidbody>().velocity.magnitude;
	...
	} // close public class Ambulance02Camera
