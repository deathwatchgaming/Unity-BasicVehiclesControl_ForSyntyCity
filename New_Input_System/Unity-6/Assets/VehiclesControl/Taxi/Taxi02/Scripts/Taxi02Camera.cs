...
namespace VehiclesControl
{
	// public class Taxi02Camera
	public class Taxi02Camera : MonoBehaviour 
	{
		// Transforms
		[Header("Transforms")]
		...
		private void LateUpdate()
		{
			...
			// float _desiredHeight is _taxi02.position.y plus _height
			float _desiredHeight = _taxi02.position.y + _height;
			...
			// transform position is _taxi02.position
			transform.position = _taxi02.position;
		...
		private void FixedUpdate()
		{
			...
			Vector3 _localVelocity = _taxi02.InverseTransformDirection(_taxi02.GetComponent<Rigidbody>().linearVelocity);
			...
			if (_localVelocity.z < -0.1f)
			{
				...
				// _tempVector3.y is _taxi02.eulerAngles.y plus 180
				_tempVector3.y = _taxi02.eulerAngles.y + 180;
			...
			else
			{
				...
				// _tempVector3 y is _taxi02.eulerAngles.y
				_tempVector3.y = _taxi02.eulerAngles.y;
			...
			float _autoCamControl = _taxi02.GetComponent<Rigidbody>().linearVelocity.magnitude;
	...
	} // close public class Taxi02Camera
