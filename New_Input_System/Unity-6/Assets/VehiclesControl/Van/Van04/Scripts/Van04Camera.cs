...
namespace VehiclesControl
{
	// public class Van04Camera
	public class Van04Camera : MonoBehaviour 
	{
		// Transforms
		[Header("Transforms")]
		...
		private void LateUpdate()
		{
			...
			// float _desiredHeight is _van04.position.y plus _height
			float _desiredHeight = _van04.position.y + _height;
			...
			// transform position is _van04.position
			transform.position = _van04.position;
		...
		private void FixedUpdate()
		{
			...
			Vector3 _localVelocity = _van04.InverseTransformDirection(_van04.GetComponent<Rigidbody>().linearVelocity);
			...
			if (_localVelocity.z < -0.1f)
			{
				...
				// _tempVector3.y is _van04.eulerAngles.y plus 180
				_tempVector3.y = _van04.eulerAngles.y + 180;
			...
			else
			{
				...
				// _tempVector3 y is _van04.eulerAngles.y
				_tempVector3.y = _van04.eulerAngles.y;
			...
			float _autoCamControl = _van04.GetComponent<Rigidbody>().linearVelocity.magnitude;
	...
	} // close public class Van04Camera
