...
namespace NavigationControl
{
	// public class Van04Compass
	public class Van04Compass : MonoBehaviour
	{
		// Header
		[Header("Raw Images")]
		...
		[Header("Transform")]
			...
			// Tooltip
			[Tooltip("The Van04 transform")]
			// Transform _van04
			[SerializeField] private Transform _van04;
		...
		public static Van04Compass _van04Compass;
		...
		private void Update()
		{
			...
			if (compassEnabled == true)
			{
				...
				// Get Component Van04Compass is enabled
				GetComponent<Van04Compass>().enabled = true;

				// Debug Log Van04 compass is enabled
			...
			{
				// Debug Log Van04 compass is disabled
				...
				// Get Component Van04Compass is not enabled
				GetComponent<Van04Compass>().enabled = false;

			} // close else if compassEnabled is false

			// Get a handle on the image's uvRect

			// _compassForeground uvRect is new Rect _van04 localEulerAngles.y 360, 0, 1, 1
			_compassForeground.uvRect = new Rect(_van04.localEulerAngles.y / 360, 0, 1, 1);

			// Get a copy of the _van04 forward vector

			// Vector3 forward is _van04 transform forward
			Vector3 forward = _van04.transform.forward;

			// Zero out the Y of _van04 forward vector to only get the direction in the X,Z 
	...
	} // close public class Van04Compass
