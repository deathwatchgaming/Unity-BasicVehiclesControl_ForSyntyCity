...
namespace NavigationControl
{
	// public class Van01Compass
	public class Van01Compass : MonoBehaviour
	{
		// Header
		[Header("Raw Images")]
		...
		[Header("Transform")]
			...
			// Tooltip
			[Tooltip("The Van01 transform")]
			// Transform _van01
			[SerializeField] private Transform _van01;
		...
		public static Van01Compass _van01Compass;
		...
		private void Update()
		{
			...
			if (compassEnabled == true)
			{
				...
				// Get Component Van01Compass is enabled
				GetComponent<Van01Compass>().enabled = true;

				// Debug Log Van01 compass is enabled
			...
			{
				// Debug Log Van01 compass is disabled
				...
				// Get Component Van01Compass is not enabled
				GetComponent<Van01Compass>().enabled = false;

			} // close else if compassEnabled is false

			// Get a handle on the image's uvRect

			// _compassForeground uvRect is new Rect _van01 localEulerAngles.y 360, 0, 1, 1
			_compassForeground.uvRect = new Rect(_van01.localEulerAngles.y / 360, 0, 1, 1);

			// Get a copy of the _van01 forward vector

			// Vector3 forward is _van01 transform forward
			Vector3 forward = _van01.transform.forward;

			// Zero out the Y of _van01 forward vector to only get the direction in the X,Z 
	...
	} // close public class Van01Compass
