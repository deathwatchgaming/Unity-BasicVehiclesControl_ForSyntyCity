...
namespace NavigationControl
{
	// public class Van03Compass
	public class Van03Compass : MonoBehaviour
	{
		// Header
		[Header("Raw Images")]
		...
		[Header("Transform")]
			...
			// Tooltip
			[Tooltip("The Van03 transform")]
			// Transform _van03
			[SerializeField] private Transform _van03;
		...
		public static Van03Compass _van03Compass;
		...
		private void Update()
		{
			...
			if (compassEnabled == true)
			{
				...
				// Get Component Van03Compass is enabled
				GetComponent<Van03Compass>().enabled = true;

				// Debug Log Van03 compass is enabled
			...
			{
				// Debug Log Van03 compass is disabled
				...
				// Get Component Van03Compass is not enabled
				GetComponent<Van03Compass>().enabled = false;

			} // close else if compassEnabled is false

			// Get a handle on the image's uvRect

			// _compassForeground uvRect is new Rect _van03 localEulerAngles.y 360, 0, 1, 1
			_compassForeground.uvRect = new Rect(_van03.localEulerAngles.y / 360, 0, 1, 1);

			// Get a copy of the _van03 forward vector

			// Vector3 forward is _van03 transform forward
			Vector3 forward = _van03.transform.forward;

			// Zero out the Y of _van03 forward vector to only get the direction in the X,Z 
	...
	} // close public class Van03Compass
