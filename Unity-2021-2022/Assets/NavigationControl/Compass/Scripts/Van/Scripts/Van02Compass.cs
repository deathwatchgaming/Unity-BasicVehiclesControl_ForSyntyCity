...
namespace NavigationControl
{
	// public class Van02Compass
	public class Van02Compass : MonoBehaviour
	{
		// Header
		[Header("Raw Images")]
		...
		[Header("Transform")]
			...
			// Tooltip
			[Tooltip("The Van02 transform")]
			// Transform _van02
			[SerializeField] private Transform _van02;
		...
		public static Van02Compass _van02Compass;
		...
		private void Update()
		{
			...
			if (compassEnabled == true)
			{
				...
				// Get Component Van02Compass is enabled
				GetComponent<Van02Compass>().enabled = true;

				// Debug Log Van02 compass is enabled
			...
			{
				// Debug Log Van02 compass is disabled
				...
				// Get Component Van02Compass is not enabled
				GetComponent<Van02Compass>().enabled = false;

			} // close else if compassEnabled is false

			// Get a handle on the image's uvRect

			// _compassForeground uvRect is new Rect _van02 localEulerAngles.y 360, 0, 1, 1
			_compassForeground.uvRect = new Rect(_van02.localEulerAngles.y / 360, 0, 1, 1);

			// Get a copy of the _van02 forward vector

			// Vector3 forward is _van02 transform forward
			Vector3 forward = _van02.transform.forward;

			// Zero out the Y of _van02 forward vector to only get the direction in the X,Z 
	...
	} // close public class Van02Compass
