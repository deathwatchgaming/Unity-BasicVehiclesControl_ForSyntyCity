/*
*  Name: Taxi 04 Compass
*  File: Taxi04Compass.cs
...
namespace NavigationControl
{
	// public class Taxi04Compass
	public class Taxi04Compass : MonoBehaviour
	{
		// Header
		[Header("Raw Images")]
		...
		public static Taxi04Compass _taxi04Compass;
		...
		private void Update()
		{
			...
			if (compassEnabled == true)
			{
				...
				// Get Component Taxi04Compass is enabled
				GetComponent<Taxi04Compass>().enabled = true;

				// Debug Log Taxi04 compass is enabled
			...
			{
				// Debug Log Taxi04 compass is disabled
				...
				// Get Component Taxi04Compass is not enabled
				GetComponent<Taxi04Compass>().enabled = false;

			} // close else if compassEnabled is false

			// Get a handle on the image's uvRect

			// _compassForeground uvRect is new Rect _taxi04 localEulerAngles.y 360, 0, 1, 1
			_compassForeground.uvRect = new Rect(_taxi04.localEulerAngles.y / 360, 0, 1, 1);

			// Get a copy of the _taxi04 forward vector

			// Vector3 forward is _taxi04 transform forward
			Vector3 forward = _taxi04.transform.forward;

			// Zero out the Y of _taxi04 forward vector to only get the direction in the X,Z 
	...
	} // close public class Taxi04Compass
