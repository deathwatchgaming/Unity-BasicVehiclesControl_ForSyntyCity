/*
*  Name: Taxi 01 Compass
*  File: Taxi01Compass.cs
...
namespace NavigationControl
{
	// public class Taxi01Compass
	public class Taxi01Compass : MonoBehaviour
	{
		// Header
		[Header("Raw Images")]
		...
		[Header("Transform")]
			...
			// Tooltip
			[Tooltip("The Taxi01 transform")]
			// Transform _taxi01
			[SerializeField] private Transform _taxi01;
		...
		public static Taxi01Compass _taxi01Compass;
		...
		private void Update()
		{
			...
			if (compassEnabled == true)
			{
				...
				// Get Component Taxi01Compass is enabled
				GetComponent<Taxi01Compass>().enabled = true;

				// Debug Log Taxi01 compass is enabled
			...
			{
				// Debug Log Taxi01 compass is disabled
				...
				// Get Component Taxi01Compass is not enabled
				GetComponent<Taxi01Compass>().enabled = false;

			} // close else if compassEnabled is false

			// Get a handle on the image's uvRect

			// _compassForeground uvRect is new Rect _taxi01 localEulerAngles.y 360, 0, 1, 1
			_compassForeground.uvRect = new Rect(_taxi01.localEulerAngles.y / 360, 0, 1, 1);

			// Get a copy of the _taxi01 forward vector

			// Vector3 forward is _taxi01 transform forward
			Vector3 forward = _taxi01.transform.forward;

			// Zero out the Y of _taxi01 forward vector to only get the direction in the X,Z 
	...
	} // close public class Taxi01Compass
